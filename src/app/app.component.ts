...
export class AppComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    private taskService: AppserviceService
  ) { }
  ...
  setTableResize(tableWidth: number) {
    ...
    this.columns.forEach(( column) => {
      if(this.taskService.getColumnSize(column.field)) {
        column.width = this.taskService.getColumnSize(column.field);
      } else {
        column.width *= scale;
      }
  ...
  setColumnWidth(column: any) {
    ...
    columnEls.forEach(( el: HTMLDivElement ) => {
      ...
      this.taskService.setColumnSize(column.field, column.width)
    });
  }

  drop(event: CdkDragDrop<string[]>) {
    moveItemInArray(this.displayedColumns, event.previousIndex, event.currentIndex);
    this.taskService.setDisplayCollumns(this.displayedColumns);
  }
}